...
function custom_meta_box_field( $field, $meta = null, $repeatable = null ) {
	...
	switch( $type ) {
		...
		// image
		case 'image':
			$image = plugin_dir_url( __FILE__ ) . '/admin/images/image.png';	
	...
	function admin_enqueue_scripts() {
		...
		if ( in_array( $pagenow, array( 'post-new.php', 'post.php' ) ) && in_array( get_post_type(), $this->page ) ) {
			...
			if ( in_array( true, array(
				...
				wp_register_script( 'chosen', plugin_dir_url( __FILE__ ) . '/admin/js/chosen.js', array( 'jquery' ) );
				$deps[] = 'chosen';
				wp_enqueue_style( 'chosen', plugin_dir_url( __FILE__ ) . '/admin/css/chosen.css' );
			}
			if ( in_array( true, array( 
				...
				wp_enqueue_script( 'meta_box', plugin_dir_url( __FILE__ ) . '/admin/js/scripts.js', $deps );
			
			// css
			$deps = array();
			wp_register_style( 'jqueryui', plugin_dir_url( __FILE__ ) . '/admin/css/jqueryui.css' );
			if ( meta_box_find_field_type( 'date', $this->fields ) || meta_box_find_field_type( 'slider', $this->fields ) )
				$deps[] = 'jqueryui';
			if ( meta_box_find_field_type( 'color', $this->fields ) )
				$deps[] = 'farbtastic';
			wp_enqueue_style( 'meta_box', plugin_dir_url( __FILE__ ) . '/admin/css/meta_box.css', $deps );
		}
	}
